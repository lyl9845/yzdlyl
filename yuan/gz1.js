...
const aesTool = {
    ...
    decrypt: (encryptedHex, keyStr = 'mvXBSW7ekreItNsT', ivStr = '2U3IrJL8szAKp0Fj') => {
        try {
...
const rsaTool = {
    decode: (data) => {
        ...
        try {
            ...
            for (let i = 0; i < buffer.length; i += blockSize) {
                ...
                try {
                    ...
                    try {
                        ...
                        OmniBox.log('error', `[瓜子APP] RSA解密失败: ${decryptErr.message}, ${fallbackErr.message}`);
                    }
                }
            }
            ...
            OmniBox.log('error', `[瓜子APP] RSA解密过程错误: ${e.message}`);
            return null;
        }
    }
};
        ...
        if (typeof response.data === 'string') {
            try {
                ...
                if (decrypted) {
                    ...
                    // 解密也失败，尝试AES解密
                    try {
                        const decrypted = aesTool.decrypt(response.data.toLowerCase());
                        if (decrypted) {
                            result = JSON.parse(decrypted);
                        } else {
        ...
        if (result.data && typeof result.data === 'string') {
            ...
            if (rsaDecrypted) {
                try {
                    ...
                    try {
                        const aesDecrypted = aesTool.decrypt(rsaDecrypted.toLowerCase());
                        if (aesDecrypted) {
                ...
                // RSA解密失败，尝试AES解密
                try {
                    const aesDecrypted = aesTool.decrypt(result.data.toLowerCase());
                    if (aesDecrypted) {
                        result.data = JSON.parse(aesDecrypted);
                    }
                } catch (e3) {
